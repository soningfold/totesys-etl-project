...
def s3(aws_credentials):
    """Mocked S3 client with no objects."""
    with mock_aws():
        s3 = boto3.client("s3")
        s3.create_bucket(
            Bucket="totesys-processed-data-000000",
...
@pytest.fixture(scope="function")
def s3_star_schema(aws_credentials):
...
class TestStarSchema:
    ...
    def test_source_data_parquet_does_not_exist(self, s3_star_schema):
        ...
        assert "/history/YYYY/MM/DD/HH:MM:SS/fact_sales_order.parquet" in object_list
        assert "/history/YYYY/MM/DD/HH:MM:SS/dim_staff.parquet" in object_list
        assert "/history/YYYY/MM/DD/HH:MM:SS/dim_date.parquet" in object_list
        assert "/history/YYYY/MM/DD/HH:MM:SS/dim_counterparty.parquet" in object_list
        assert "/history/YYYY/MM/DD/HH:MM:SS/dim_currency.parquet" in object_list
        assert "/history/YYYY/MM/DD/HH:MM:SS/dim_design.parquet" in object_list
        assert "/history/YYYY/MM/DD/HH:MM:SS/dim_location.parquet" in object_list

    @pytest.mark.it(
        "Inserts new data into star schema database if parquet files exists"
    )
